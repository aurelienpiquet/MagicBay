...
def chat_page(username, receiver, message_id):
    ...
    if request.method == 'POST' and chat_form.validate_on_submit():
        ...
        if turbo.can_stream():
            ...
            return turbo.stream([
                   turbo.append(
                       render_template('chat/_message.html', message=message, now=datetime.datetime.now()), target='message-chat'),
                   ])
    ...
    return render_template('chat/chat.html', chat_form=chat_form, username=username, receiver=receiver, messages=messages, now=datetime.datetime.now())
